...
const main = async () => {
  ...
  cliParser.addArgument(['--path-ffprobe'], {help: 'Path to the ffprobe binary.', dest: 'pathFFProbe', defaultValue: `/usr/local/bin/ffprobe`})
  cliParser.addArgument(['--path-say'], {help: 'Path to the say binary.', dest: 'pathSay', defaultValue: `/usr/bin/say`})
  cliParser.addArgument(['--path-node'], {help: 'Path to the Node binary.', dest: 'pathNode', defaultValue: `/usr/local/bin/node`})
  cliParser.addArgument(['--path-bnetdata'], {help: 'Path to the bnetdata binary.', dest: 'pathBnetdata', defaultValue: `${os.homedir()}/Utilities/bnetdata/index.js`})
