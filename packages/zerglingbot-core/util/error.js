...
const extractErrorFields = error => {
  ...
  const errorValue = errorMember(error, 'error')
  ...
  if (errorValue && isString(errorValue))
    fields['error'] = ['Error', `${errorValue}`, true]
  if (code)
...
class InternalError extends Error {
  constructor(args) {
    ...
    this.code = args?.code ?? `ZERGLINGBOT_E_INTERNAL`
    this.isInternalError = this.code.startsWith('ZERGLINGBOT')
